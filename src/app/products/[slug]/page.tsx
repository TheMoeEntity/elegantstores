...
const Products = async ({ params }: { params: { slug: string } }) => {
  ...
  const justIn =
    (await Helpers.getProducts("https://fakestoreapi.com/products")) ?? [];
  let single: any = await Helpers.getSingleProduct(params.slug as string);
  if (!single) {
    let single2: any = await Helpers.getSingle(params.slug).then(
      async (prod) => {
        if (prod) {
  ...
  const oldwishlist = (await Helpers.fetchSupabaseUsers()
    .then((x) => x.wishlist.items)
    .catch(() => [])) as wishList[];
