...
const ShopPage = ({ products }: { products: ISBProducts[] }) => {
  ...
  const priceFilterAction = () => {
    ...
    filtered = Helpers.filterProductsByPrice(
      products,
      selectedPriceRanges?.min,
      selectedPriceRanges.max
    ).filter((item) => (active === "all" ? item : item.category === active));
    setLoading(false);
    return filtered;
  };
  const searchAction = (categoryy: string) => {
    let filtered: any = items;
    filtered = Helpers.filterCategory(categoryy, products).filter(
      (item) =>
        item.price >= selectedPriceRanges.min &&
        item.price <= selectedPriceRanges.max
    );
    setLoading(false);
    return filtered;
  };
  ...
  const handleCheckboxChange = (min: number, max: number, id: number) => {
    ...
    setPriceRanges((state) => {
      const newRange = state.map((range) =>
        range.id === id
  ...
  return (
    <section>
      <div className="hero w-full relative h-[300px] md:h-[500px]">
        ...
        <div className="absolute text-center flex flex-col gap-y-10 z-20 text-white w-[90%] h-[70%] md:h-[50%] top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
          <h1 className="text-center text-xl">
            {" "}
            Home <i className="fa-solid fa-angle-right mx-3"></i>{" "}
            <b className="text-[#377DFF]">Shop</b>
          </h1>
          <h1 className="text-center font-semibold text-3xl md:text-5xl underline underline-offset-8">
            Shop Page
          </h1>
      ...
      <div className="flex my-8 flex-row gap-3 flex-wrap mx-auto w-[95%]">
        <div
          className={
            (hideSideBar ? "-translate-x-[100%]" : "translate-x-0 py-6") +
            " trans shadow-2xl top-0 w-[250px] px-5 md:px-0 md:w-auto h-full flex pt-10 md:pt-0 overflow-y-scroll left-0 bg-[#F9FBFD] md:bg-transparent md:shadow-none z-[10000] fixed md:static md:flex basis-[23%] md:h-[800px] gap-y-9 flex-col"
          }
        >
          <button
            onClick={() => setHideSideBar(true)}
            className="md:hidden text-4xl absolute top-5 right-5"
          >
            &times;
          </button>
          ...
          <div>
            <ul className="text-gray-600 gap-y-5 flex-col flex w-fit">
              <button
                className={`w-fit ${active === "all" && "text-black underline"}`}
                onClick={() => categoryFilter("all")}
              >
                All Products
              </button>
              <button
                className={`w-fit ${active === "shoes" && "text-black underline"}`}
                onClick={() => categoryFilter("shoes")}
              >
                Shoes
              </button>
              <button
                className={`w-fit ${active === "shirts" && "text-black underline"}`}
                onClick={() => categoryFilter("shirts")}
              >
                Shirts
              </button>
              <button
                className={`w-fit ${active === "pants" && "text-black underline"}`}
                onClick={() => categoryFilter("pants")}
              >
                Pants
              </button>
              <button
                className={`w-fit ${active === "jackets" && "text-black underline"}`}
                onClick={() => categoryFilter("jackets")}
              >
                Jackets
              </button>
              <button
                className={`w-fit ${active === "hoodies" && "text-black underline"}`}
                onClick={() => categoryFilter("hoodies")}
              >
                Hoodies
              </button>
            </ul>
          ...
          <div>
            <form className="text-gray-600 gap-y-5 flex-col flex">
              {priceRanges.map((range, index) =>
                index === 0 ? (
                  <label
                    className="w-full md:text-sm lg:text-[18px] flex justify-between"
                    key={index}
                  >
                    All Prices
                    <input
                      ...
                      onChange={() =>
                        handleCheckboxChange(range.min, range.max, range.id)
                      }
                    />
                  </label>
                ) : (
        ...
        <div className="md:basis-[72%] basis-full">
          <div className="my-8 flex md:hidden justify-between border-b-[1px] border-t-[1px] border-gray-300 py-6 mx-auto w-[90%]">
            <div className="text-xl">
              ...
              <button onClick={() => setHideSideBar(false)}>Filter</button>
            </div>
          ...
          <div className="text-xl mx-auto my-7 w-[90%] flex justify-between ">
            ...
            <div>
              <select className="custom-select text-sm outline-none w-full px-2 py-2 text-[black] font-semiold border-[1px] border-[#eef5ff]">
              ...
              <div className="flex flex-row gap-2 gap-y-7 md:gap-y-5 md:gap-4 md:justify-center justify-start flex-wrap mb-7">
                {items.length === 0 ? (
                  ...
                  items.map((x) => (
                    <div
                      key={x.id}
                      className="flex group overflow-y-hidden pb-4 md:min-h-auto shadow-md rounded-md flex-col gap-y-2 gap-x-0 h-fit md:min-w-[10%] basis-[47%] md:basis-[30%] lg:basis-[23%]"
                    >
                      ...
                      <Link
                        ...
                        className="font-semibold px-4 mt-1"
                      >
                        <span className="hover:text-[#377DFF]">{x.title}</span>
                      </Link>
                      <div className="px-3 font-semibold mt-1">
                      ...
                      <div className="w-[90%] mt-1 transition-transform duration-[0.55s] ease mx-auto group-hover:translate-y-0 translate-y-20">
                        <button
                          onClick={() => addAction(x)}
                          className="w-full px-3 py-2 bg-black text-white rounded-lg"
                        >
                          Add to cart
                        </button>
                      </div>
